...
class FlickrImageDownload:
    def __init__(self):
        ...
        self.flickr = flickrapi.FlickrAPI(
            self.config["FLICKR"]["id"], self.config["FLICKR"]["secret"], cache=True
        )
    ...
    def load_image(self, url):
        try:
            ...
            logging.warning(
                f"Unexpected exception while downloading image: {url}", exc_info=True
            )
            return None, None
    ...
    def check_to_keep_photo(self, url, image):
        ...
        p = os.path.join(
            self.config_path, f"{self.config_prefix}-{h}.{self.config_format}"
        )
